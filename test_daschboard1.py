...
client_id = st.number_input("Enter the client ID:", value=0, step=1)   
if st.button('info_client'):
    ...
    def gauge_chart(thres):
            ...
            #client_data = pd.DataFrame(response_data['client'])
            client_data = data_test.query("index == @client_id")
...
if st.button('Analyse bivariée'):
 ...
 #------------------------------------------------Nouveau client, veuillez rentrer les variables-----------------------------------------------------
init = st.markdown("Si vous rentrer un nouveau veuillez remplir les infrmations clients et taper Nouveau_client")
...
source_1 = st.sidebar.number_input('EXT_SOURCE_1', -4.0, 4.0, step=0.0001, format="%.4f")
source_2 = st.sidebar.number_input('EXT_SOURCE_2', -10.0, 10.0,step=0.0001, format="%.4f")
ext_source_3 = st.sidebar.number_input('EXT_SOURCE_3', -4.0, 4.0, step=0.0001, format="%.4f")

if st.button('Nouveau_client'):
    def get_predict_of_id1():
    #Get the client ID from the user
    ...
    def gauge_chart1(thres):
        #client_id, response_data = get_predict_of_id()
        data, response_data = get_predict_of_id1()
